...
static void main_loop(void)
{
   ...
   for ( ;; )
   {
      ...
      for ( u = 0 ; u < pset_count( SERVICES( ps ) ) ; u++ )
      {
         ...
         {
//RCP
	// initialize shared memory - if NULL still continue
	if (shm == NULL)
		shm = rcpShmemInit(RCP_PROC_CLI);
	// increment connection counters
         	if (shm != NULL) {
                	if (strcmp(sp->svc_conf->sc_name, "tftp") == 0)
                		shm->stats.tftp_rx++;
                	else if (strcmp(sp->svc_conf->sc_name, "rcptelnet") == 0)
                		shm->stats.telnet_rx++;
                	else if (strcmp(sp->svc_conf->sc_name, "rcpftp") == 0)
                		shm->stats.ftp_rx++;
                	else if (strcmp(sp->svc_conf->sc_name, "rcphttp") == 0)
                		shm->stats.http_rx++;
  		else if (strcmp(sp->svc_conf->sc_name, "rcpssh") == 0)
                		shm->stats.sec_rx++;
                }
//RCP                
