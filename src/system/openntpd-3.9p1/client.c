...
//RCP
		rcpLog(muxsock, RCP_PROC_NTPD, RLOG_INFO, RLOG_FC_NTPD,
		    "reply from %s: not synced, next query %ds",
...
//RCP - update shared memory
	if (p->rcp_server == NULL) {
		struct sockaddr_in *s = (struct sockaddr_in *) &p->addr->ss;
		uint32_t ip = ntohl(s->sin_addr.s_addr);
		int i;
		for (i = 0, p->rcp_server = &shm->config.ntp_server[0]; i < RCP_NTP_LIMIT; i++, p->rcp_server++) {
			if (p->rcp_server->valid == 0)
				continue;
			if (p->rcp_server->ip == ip)
				break;
		}
		if (i == RCP_NTP_LIMIT) {
			for (i = 0, p->rcp_server = &shm->config.ntp_server[0]; i < RCP_NTP_LIMIT; i++, p->rcp_server++) {
				if (p->rcp_server->valid == 0)
					continue;
				if (strcmp(p->rcp_server->name, p->addr_head.name) == 0)
					break;
			}
			if (i == RCP_NTP_LIMIT)
				p->rcp_server = NULL;
	...
	if (p->rcp_server != NULL) {
		p->rcp_server->delay = p->reply[p->shift].delay;
		p->rcp_server->offset = p->reply[p->shift].offset;
	}
...
//RCP
		if (p->rcp_server != NULL)
			p->rcp_server->interval = interval;
		rcpLog(muxsock, RCP_PROC_NTPD, RLOG_INFO, RLOG_FC_NTPD,
			"reply from %s: negative delay %f, next query %ds",
...
//RCP
	if (p->rcp_server != NULL) {
		p->rcp_server->interval = interval;
		p->rcp_server->stratum = p->reply[p->shift].status.stratum;
		p->rcp_server->trustlevel = p->trustlevel;
	}
		rcpLog(muxsock, RCP_PROC_NTPD, RLOG_DEBUG, RLOG_FC_NTPD,
	"reply from %s: offset %f delay %f, "
