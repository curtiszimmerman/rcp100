...
"conf_main : SERVERS address",
"conf_main : SERVER address",
"address : STRING",
...
#line 79 "./parse.y"
	{
			...
			if ((h = yystack.l_mark[0].v.addr->a) == NULL &&
			    (host_dns(yystack.l_mark[0].v.addr->name, &h) == -1 || !h)) {
				yyerror("could not resolve \"%s\"", yystack.l_mark[0].v.addr->name);
