...
#include "ntp.h"
//RCP
#include "../../include/librcp_ip.h"
RcpShm *shm = NULL;
int muxsock = 0;		// RCP services sockets
double last_offset_median;
//RCP
...
void usr1_handler(int sig) {
	struct ntp_peer		*p;
	log_debug("%d peers configured", peer_cnt);
	log_debug("conf->status.synced %d", conf->status.synced);
	...
	TAILQ_FOREACH(p, &conf->ntp_peers, entry) {
		...
		struct sockaddr_in *s = (struct sockaddr_in *) &p->addr->ss;
log_debug("%d.%d.%d.%d", RCP_PRINT_IP(ntohl(s->sin_addr.s_addr)));

		log_debug("%s: trust %d, offset %f delay %f",
...
// RCP: connect rcp socket
	// initialize shared memory
	if ((shm = rcpShmemInit(RCP_PROC_CLI)) == NULL)
		fatal("rcpShmemInit");
	// connect multiplexer
	muxsock = rcpConnectMux(RCP_PROC_NTPD, NULL, NULL, NULL);	
	if (muxsock == 0)
		fatal("rcpConnectMux");
//
...
//RCP
	// use SIGUSR1 to print peer statistics	
	signal(SIGUSR1, (sighandler_t) usr1_handler);
//RCP	
	close(pipe_prnt[0]);
...
//RCP
		rcpLog(muxsock, RCP_PROC_NTPD, RLOG_INFO, RLOG_FC_NTPD,
	"ntp engine ready");
...
//RCP
			rcpLog(muxsock, RCP_PROC_NTPD, RLOG_INFO, RLOG_FC_NTPD,
				"adjusting local clock by %fs", last_offset_median);
			shm->config.ntp_offset = last_offset_median;	
//RCP			
			memcpy(&n, imsg.data, sizeof(n));
			if (n == 1 && !conf->status.synced) {
//RCP
		rcpLog(muxsock, RCP_PROC_NTPD, RLOG_NOTICE, RLOG_FC_NTPD,
				"clock is now synced");
...
//RCP
		last_offset_median = offset_median;
		shm->config.ntp_offset = offset_median;
		shm->config.ntp_rootdelay = conf->status.rootdelay;
		shm->config.ntp_rootdispersion = conf->status.rootdispersion;
		shm->config.ntp_reftime = conf->status.reftime;
		shm->config.ntp_synced = conf->status.synced;
		shm->config.ntp_stratum = conf->status.stratum;
		shm->config.ntp_refid = ntohl(conf->status.refid);
//RCP		
