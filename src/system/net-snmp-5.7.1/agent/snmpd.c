 ...
 */
 //RCP - monitoring snmp statistics
#include "../../../include/librcp_snmp.h"
RcpSnmpStats rstats;
int rstats_initialized = 0;

void catch_alarm (int sig) {
	if (!rstats_initialized) {
		memset(&rstats, 0, sizeof(rstats));
		rstats_initialized = 1;
	}
	
	// get the new statistics
	int modified = 0;
	// packets in
	unsigned value = snmp_get_statistic(STAT_SNMPINPKTS);
	if (value != rstats.inpkts) {
		rstats.inpkts = value;
		modified = 1;
	}

	// packet type
	value = snmp_get_statistic(STAT_SNMPINGETREQUESTS);
	if (value != rstats.ingetrq) {
		rstats.ingetrq = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPINGETNEXTS);
	if (value != rstats.innextrq) {
		rstats.innextrq = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPINSETREQUESTS);
	if (value != rstats.insetrq) {
		rstats.insetrq = value;
		modified = 1;
	}
	
	// input errors
	value = snmp_get_statistic(STAT_SNMPINBADVERSIONS);
	if (value != rstats.inerrbadver) {
		rstats.inerrbadver = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPINBADCOMMUNITYNAMES);
	if (value != rstats.inerrbadcomname) {
		rstats.inerrbadcomname = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPINBADCOMMUNITYUSES);
	if (value != rstats.inerrbadcomuses) {
		rstats.inerrbadcomuses = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPINASNPARSEERRS);
	if (value != rstats.inerrasn) {
		rstats.inerrasn = value;
		modified = 1;
	}
	
	// output packets
	value = snmp_get_statistic(STAT_SNMPOUTPKTS);
	if (value != rstats.outpkts) {
		rstats.outpkts = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPOUTGETRESPONSES);
	if (value != rstats.outgetresp) {
		rstats.outgetresp = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPOUTTOOBIGS);
	if (value != rstats.outerrtoobig) {
		rstats.outerrtoobig = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPOUTNOSUCHNAMES);
	if (value != rstats.outerrnosuchname) {
		rstats.outerrnosuchname = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPOUTBADVALUES);
	if (value != rstats.outerrbadvalue) {
		rstats.outerrbadvalue = value;
		modified = 1;
	}
	value = snmp_get_statistic(STAT_SNMPOUTGENERRS);
	if (value != rstats.outerrgenerr) {
		rstats.outerrgenerr = value;
		modified = 1;
	}
	...
	if (modified) {
		...
		int fd = open(RCPSNMP_STATS_FILE, O_CREAT | O_WRONLY | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
		if (fd != -1) {
			write(fd, &rstats, sizeof(rstats));
			close(fd);
		}
	}
