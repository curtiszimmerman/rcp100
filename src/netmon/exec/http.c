...
void monitor_http(void) {
	...
	for (i = 0, ptr = shm->config.netmon_host; i < RCP_NETMON_LIMIT; i++, ptr++) {
		if (!ptr->valid || ptr->type != RCP_NETMON_TYPE_HTTP)
			continue;
			...
			if (ptr->wait_response == 0) {
				if (ptr->status == 0) {
					...
					rcpLog(muxsock, RCP_PROC_NETMON, RLOG_NOTICE, RLOG_FC_NETMON,
						"HTTP server %s %u UP", ptr->hostname, port);
				}
			}
			else {
				if (ptr->status == 1) {
					ptr->status = 0;
					rcpLog(muxsock, RCP_PROC_NETMON, RLOG_NOTICE, RLOG_FC_NETMON,
						"HTTP server %s %u DOWN", ptr->hostname, port);
				}
				ptr->wait_response = 0;
				ptr->sock = 0;
			}
...
void http_flag_update(void) {
	...
	if (newflag != http_enabled)
		http_enabled = newflag;
}
